...
export function EmptyState({
  ...
  return (
    ...
    >
      ...
      <h3 className="mt-4 text-lg font-unbounded font-semibold">{title}</h3>
      {description && (
        <p className="mt-2 text-sm font-semibold font-sofia text-muted-foreground max-w-sm">
          {description}
        </p>
      )}
      {action && (
...
export function EmptyProjectsState({
  ...
  return (
    <EmptyState
      ...
      action={
        onCreateNew
          ? { label: "Create Project", onClick: onCreateNew }
          : undefined
      }
    />
  );
...
export function EmptyTasksState({ onCreateNew }: { onCreateNew?: () => void }) {
  return (
    <EmptyState
      ...
      action={
        onCreateNew ? { label: "Add Task", onClick: onCreateNew } : undefined
      }
    />
  );
}
...
export function SkeletonCard({ className }: SkeletonCardProps) {
  return (
    <div
      className={cn("rounded-lg border bg-card p-4 animate-pulse", className)}
    >
